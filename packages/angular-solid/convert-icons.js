...
function convertAllIcons() {
  const solidIconsPath = path.join(__dirname, '../solid/src');
  const outputPath = path.join(__dirname, 'src');
  
  // Ler todos os arquivos .tsx
  const files = fs.readdirSync(solidIconsPath)
  ...
  files.forEach(file => {
    ...
    const reactContent = fs.readFileSync(path.join(solidIconsPath, file), 'utf8');
  ...
  const publicApiContent = `// Public API for Angular Solid Icons
${exports.join('\n')}
...
export * from './types';`;
  ...
  console.log(`Convertidos ${files.length} ícones solid para Angular`);
